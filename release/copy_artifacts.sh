...
if [ "${PROJECT}" = "kubernetes" ] && [ "${BUILD_ARTIFACTS}" == "false" ]; then
    echo "Copying checksums instead as they were generated in the internal build"
    build::common::echo_and_run cp "${BASE_DIRECTORY}/projects/kubernetes/kubernetes/_output/SHA*" "$ARTIFACT_DIR"
    exit 0
fi
